...
def derive_key(passphrase, salt):
  salt = salt if salt is not None else urandom(16)
...
def unlock_record(key, passphrase, record):
  record['plain'] = literal_eval(
    decrypt(key, record['cipher'], passphrase).decode())
  del record['cipher']
  return record
...
def unlock_data(record, passphrase):
  ...
  return (record[0],
      literal_eval(decrypt(key, ba.a2b_hex(record[1])).decode()))


def lock_data(record, passphrase):
  salt, key = derive_key(record[0].encode(), passphrase)
  return (record[0],
      ba.b2a_hex(encrypt(key, str(record[1]).encode())).decode())
