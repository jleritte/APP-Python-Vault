...
class ui:
  ...
  def __fillRecord(self, old):
    ...
    pos = self.__printPrompt(pop, (1, 1), 0, prompts)
    name = self.__textEntry(pop, pos, old[0])
    if not name:
      return
    pos = self.__printPrompt(pop, (2, 1), 1, prompts)
    pword = self.__textEntry(pop, pos, old[1][0], "*")
    if not pword:
      return
    pos = self.__printPrompt(pop, (3, 1), 2, prompts)
    uname = self.__textEntry(pop, pos, old[1][1])
    if not uname:
      return
  ...
  def __deleteRecord(self, record):
    ...
    prompt = "Are you sure?"
    x, y = pop.getmaxyx()
    pop.addstr(0, int(y/2 - len(title)/2), title)
    pop.addstr(x-1, int(y/2 - len(self.__controlstr)/2), self.__controlstr)
    pop.addstr(int(x/2), int(y/2 - len(prompt)/2), prompt)
