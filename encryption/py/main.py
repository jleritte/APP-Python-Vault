...
async def message_handle(websocket, path):
    ...
    JSON = {"key": export_public_key(ecKeys.public_key())}
    ...
    try:
        async for message in websocket:
            ...
            if action == 'login':
                ...
                success = login(cid)
                response = wrapResponse(cid, action, success, read_raw(
                    connection['filename'])[0] if success else None)
            if action == 'sync':
                success = len(connection) > 0
                response = wrapResponse(cid, action, success, read_raw(
                    connection['filename'])[1:] if success else None)
            if action == 'update':
                for entry in data:
            ...
            if action == 'password':
                ...
                response = wrapResponse(
                    cid, action, success, generate(int(data)))

            if response:
                await websocket.send(response)
                address = websocket.remote_address[0]
                log(f'{address},{cid} {action} {success}', 1)
    except websockets.exceptions.ConnectionClosedError:
        address = websocket.remote_address[0]
        log(f"Error {address},{cid} {sys.exc_info()[1]}", error=1)
    finally:
...
def start_ui():
    ...
    try:
        ...
        while True:
            ...
            exit = scr.update(
                ('print', data, ch, connections[uiUser]['password']))
            if exit is None:
                break
